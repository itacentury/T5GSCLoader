...
t5nd(XAssetHeader*, DB_FindXAssetHeader,
     (XAssetHeader * header, XAssetType type, const char* name, bool errorIfMissing,
      int waitTime));
t5nd(XAssetEntryPoolEntry*, DB_LinkXAssetEntry,
     (XAssetEntry * newEntry, int allowOverride));
t5nd(dvar_s*, Dvar_FindVar, (const char* name));
t5nd(int, Scr_GetFunctionHandle,
     (scriptInstance_t inst, const char* scriptName, const char* functionName));
...
t5nhd(int, cellSpursLFQueuePushBody,
      (CellSpursLFQueue * lfqueue, const void* buffer, unsigned int isBlocking));
