...
// allows iPrintln_GameMessage and iPrintlnBold_GameMessage by noping
// Com_GetParseThreadInfo in 0x001AF228 CL_ConsolePrint
void RemoveThreadIDCheckOnCL_ConsolePrint() {
    uint32_t PPC[] = {0x60000000};
    for (int i = 0; i < 0x4; i++) sys_dbg_process_write(0x1AF264 + (i * 4), &PPC[0], 4);
}
...
void checkScreenResolution() {
    ...
    if (res == CELL_OK) {
        ...
        printf(T5INFO "Detected screen resolution '%i×%i'", resInfo.width,
               resInfo.height);
    } else {
        width = 1280.0f;
        height = 720.0f;
        printf(T5WARNING "Unknown screen resolution (%d), using 1280×720",
               config.resolutionId);
    }
...
void WriteFloat(int Address, float Input) { *(float*)Address = Input; }
